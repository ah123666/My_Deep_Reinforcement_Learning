...
class Net(nn.Module):
    ...
    def __init__(self, n_in, n_mid, n_out):
        ...
        # Dueling Network
        self.fc3_adv = nn.Linear(n_mid, n_out)  # Advantage 部分, n_out为动作的数量
        self.fc3_v = nn.Linear(n_mid, 1)  # 价值V部分
    ...
    def forward(self, x):
        ...
        adv = self.fc3_adv(h2)
        # 让val从[batch*1]转为[batch*动作的数量]
        val = self.fc3_v(h2).expand(-1, adv.size(1))

        output = val + adv - \
            adv.mean(1, keepdim=True).expand(-1, adv.size(1))  # 这里的-1是让电脑自己计算
        ...
        for episode in range(NUM_EPISODES):
            ...
            for step in range(MAX_STEPS):
                ...
                if done and episode_final == False:
                    print("%d Episode: Finished after %d steps: = %.1lf" %
